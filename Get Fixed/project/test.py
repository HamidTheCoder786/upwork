...
def refine_subtitles_based_on_computer_vision(subtitles: pysrt.SubRipFile, timestamps: List[Dict], replacement_base_folder) -> pysrt.SubRipFile:
    ...
    if len(candidate_timestamps) != len(subtitles) - 1: # the last subtitle does not have a corresponding timestamp for it's literally the ending timestamp of the entire video
    ...
    for folder in replacement_base_folder.iterdir():
        ...
        if len(replacement_video_files) == 0:
            logging.warning(f"Found {len(replacement_video_files)} replacement video files in {folder}. Skipping...")
            continue
        
        clips.append(replace_index)
    ...
    for subtitle in subtitles:
        ...
        while cursor < len(candidate_timestamps):
            ...
            candidate['timestamp'] = candidate['timestamp'] + 0.05 # add 1/24 second to the timestamp to avoid float precision error (FPS related, only work for 24 and above fps videos)
    ...
    for i, subtitle in enumerate(subtitles):
        if i > 0:
            if i in clips and i - 1 not in clips:
                ...
                subtitle_start -= 0.1 # two~one frames back to earse the previous alteration
                subtitle.start = pysrt.SubRipTime(
